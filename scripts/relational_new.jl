...
#=
srun julia relational_new.jl --n $SLURM_ARRAY_TASK_ID --m $1
exit
# =#
using DrWatson
@quickactivate
...
function split(x::Mill.AbstractMillNode, y::AbstractArray{Ti,1}, seed::Ti=Ti(1), ratio::Array{Tr,1}=[64f-2, 16f-2, 2f-1]) where {Tr<:Real,Ti<:Int}
    Random.seed!(seed)
    i = randperm(length(y))
    n = cumsum(map(n->ceil(Ti, n), ratio*Mill.nobs(x)))
...
dirdata = "/home/martin/datasets/relational"
datasets = [
...
function estimate(config::NamedTuple)
    (; dirdata, dataset, seed_split, seed_init, pl, ps, dist_ngram, nepoc, bsize, supervision, msave) = config
    ...
    f(x) = x
    f(x::Mill.NGramMatrix) = SparseMatrixCSC(x)
    # x = Mill.mapdata(f, x)

    # x_trn, x_val, x_tst, y_trn, y_val, y_tst = split(x, y, seed_split)

    if dist_ngram == :geometric
        dist_ngram = d -> Geometric(d)
    elseif dist_ngram == :zipoisson
        dist_ngram = d -> ZIPoisson(d)
    elseif dist_ngram == :poisson
        dist_ngram = d -> Poisson(d)
    else
        @error "Usupported leaf distribution"
    end

    m = reflectinmodel(x[1], length(unique(y)); hete_nl=pl, hete_ns=ps, dist_gram=dist_ngram, seed=seed_init)
...
function slurm()
    @unpack n, m = commands()
    dataset = datasets[m]
    pl, ps, dist_ngram, nepoc, bsize, supervision, seed_split, seed_init = collect(Iterators.product(
        [1, 2, 3],
        [1, 2, 3],
        [:poisson, :geometric, :zipoisson],
        [400],
        [20], # [10, 20, 30],
        [1f-0],
        [1],
    ...
    settings = (; seed_split, seed_init, dirdata, dataset=dataset.name, pl, ps, dist_ngram, nepoc, bsize, supervision, rtype=Float32, itype=Int64, msave=false)
