...
function MvNormal(d::Integer; dtype::Type{<:Real}=Float64, μinit::Symbol=:uniform, Σinit::Symbol=:unit, Σtype::Symbol=:full, r::Real=0.)
    ...
    if Σtype == :full
        return MvNormalParams(μ, diagm(diagΣ), dtype(r))
    elseif Σtype == :diag
        return MvNormalParams(μ, diagΣ, dtype(r))
    elseif Σtype == :scalar
        return MvNormalParams(μ, diagΣ[1:1], dtype(r))
    else
        @error "Specified covariance type $(Σtype) is not supported."
    end
end

function Base.rand(m::MvNormal{T, 2}, n::Int) where {T<:Real} 
    inv(m.A) * (randn(T, length(m.b), n) .- m.b)
end
function Base.rand(m::MvNormal{T, 1}, n::Int) where {T<:Real} 
...
function logpdf(m::MvNormal{T, 2}, x::Array{T, 2}) where {T<:Real}
    z = (m.A + m.r * I) * x .+ m.b
...
function logpdf(m::MvNormal{T, 1}, x::Array{T, 2}) where {T<:Real}
    z = (m.A .+ m.r) .* x .+ m.b
...
function logpdf(m::MvNormal{T, 1}, x::Array{Maybe{T}, 2}) where {T<:Real}
    z = (m.A .+ m.r) .* x .+ m.b
