...
function reconstruct_loss(x::Array{T, 2}, x̂::Array{T, 2}; σ::T=T(0.1)) where T 
    k = size(x, 1)
    y = (x .- x̂) ./ σ
    -T(k/2) * ( T(2*pi) + log(σ)) .- T(0.5)*sum( y.^2, dims=1)
end

function elbo(m::VAE, x::AbstractArray{T}) where T
    ...
    rec = reconstruct_loss(x, m.decoder(z))
    kl = kl_loss(μ, σ)
    mean(rec - kl)
end
