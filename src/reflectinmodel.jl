...
function reflectinmodel(x, n::Int; n_set_mix::Int=1, depth_prod::Int=2, n_prod_mix::Int=2,
    ...
    kwargs = (; n_set_mix, depth_prod, n_prod_mix, f_cont, f_cat, f_card, dtype)
    SumNode([_reflectinmodel(x; kwargs...) for _ in 1:n]; dtype=dtype)
end
...
function _reflectinmodel(x::Mill.BagNode; kwargs...)
    ...
    n_set == 1 ? SetNode(f_inst(), f_card()) : SumNode(map(_->SetNode(f_inst(), f_card()), 1:n_set)...)
end
...
function _reflectinmodel(x::Mill.ProductNode{<:NamedTuple}; kwargs...)
    n_set_mix = kwargs[:n_prod_mix] 
    depth_prod = kwargs[:depth_prod]
    @assert depth_prod > 0

    if depth_prod == 1
        _build_prod(x, n_set_mix; kwargs...)
    else 
        _build_prod(x, collect(keys(x)), depth_prod, n_set_mix; kwargs...)   
    end
end
